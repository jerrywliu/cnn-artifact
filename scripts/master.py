...
def eval_CNN_regression(experiment_name, trials, inFile, image_path, categoryName, gpus, model_type, epoch_number, augment_number, learning_rate, decay, regularization, batch_size, img_height, img_width):
    ...
    with open(os.path.join(main_experiment_path, experiment_name, experiment_name+':experiment_summary.txt'), 'w') as summary_writefile:
        ...
        for i in range(1, len(means)+1):
            summary_writefile.write('Model ' + str(i) + ': ' + 'mean: ' + str(means[i-1]) + ', variance: ' + str(variances[i-1]) + '\n')
...
#model_types = ['vgg16', 'resnet', 'inception']
model_types = ['resnet', 'inception']
#numCats = [19, 25, 41]
numCats = [41]
'''
for model in model_types:
    print(eval_CNN_categorical(experiment_name='category_'+model,
                            trials=10,
                            inFile='../data/allbeijingdata.txt',
                            image_path='../../../bjpics',
...
for cats in numCats:
    for model in model_types:
        print(eval_CNN_categorical(experiment_name=str(cats)+'_types_'+model,
                            trials=10,
                            inFile='../data/allbeijingdata.txt',
                            image_path='../../../bjpics',
